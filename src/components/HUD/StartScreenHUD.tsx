...
function PlayMenu() {
  const { setCurrentSceneIndex, setShouldPlaySound, setShouldZoomCamera } = useSceneStore((state) => state);
  // const [shouldZoomCamera, setShouldZoomCamera] = useState(false);
    ...
    <div className="flex flex-col w-[380px] absolute right-8 bottom-8  bg-black bg-opacity-70 z-10">
      <div className="p-8 text-white flex flex-col items-center space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex">
            <input onChange={() => {}} checked value="1" type="checkbox" className="mr-2" />
      ...
      <button
        ...
        onMouseEnter={() => {
          setShouldZoomCamera(true);
        }}
        onMouseLeave={() => {
          setShouldZoomCamera(false);
        }}
      >
        PLAY
      </button>
